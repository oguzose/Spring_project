...
public class ApplicationFormValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (application.getApplication().getWorkStatus() == WorkStatus.NOT_WORKING
			...
			errors.rejectValue("application.workStatus",
					"error.notWorkingOfficer",
					"Hem çalışmayıp hem nasıl memursun!");
		}
		// check course size
		application.getPreferredCourseIds().removeAll(
				Collections.singleton(null));
		if (application.getPreferredCourseIds().size() == 0) {
			errors.rejectValue("preferredCourseIds",
					"error.preferredCourseNotSelected",
		...
		if (listSize != setSize) {
			errors.rejectValue("preferredCourseIds[0]",
					"error.preferredCourseSame", "Aynı kursu seçemezsiniz!");
		}
		...
		boolean tcknValid = this.tcknValidationService.validate(
				student.getName(), student.getSurname(),
				student.getBirthDate(), student.getTckn());
		if (!tcknValid) {
			errors.rejectValue("student.tckn", "error.tcknValidate",
					"Tc Kimlik no hatalı.");
		}

		boolean checkBlack = this.blackListService.validate(student.getTckn(),
				student.getEmail(), student.getName(), student.getSurname());
		if (!checkBlack) {
			errors.rejectValue("student.email", "error.checkBlackList",
					"Kara listedesiniz!");
		}

		// blacklist check
		boolean checkExam = this.examValidationService.validate(
				student.getTckn(), student.getEmail());
		if (!checkBlack) {
			errors.rejectValue("student.email", "error.checkExam",
